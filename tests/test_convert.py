...
def test_organize_by_notebook(tmp_path, monkeypatch):
    notebook_path = tmp_path / "the_notebook.ipynb"
    sentinel_path = tmp_path / SAVE_PROGRESS_INDICATOR_FILE
    shutil.copy(NOTEBOOK_FILE, notebook_path)
...
def test_organize_by_extension(tmp_path, monkeypatch):
    notebook_path = tmp_path / "the_notebook.ipynb"
    sentinel_path = tmp_path / SAVE_PROGRESS_INDICATOR_FILE
    shutil.copy(NOTEBOOK_FILE, notebook_path)
    with sentinel_path.open("w") as fp:
...
def test_type_file(tmp_path, monkeypatch):
    notebook_path = tmp_path / "the_notebook.ipynb"
    sentinel_path = tmp_path / SAVE_PROGRESS_INDICATOR_FILE
    shutil.copy(NOTEBOOK_FILE, notebook_path)
    with sentinel_path.open("w") as fp:
...
def test_type_directory(tmp_path, monkeypatch):
    notebook_path = tmp_path / "the_notebook.ipynb"
    sentinel_path = tmp_path / SAVE_PROGRESS_INDICATOR_FILE
    shutil.copy(NOTEBOOK_FILE, notebook_path)
    with sentinel_path.open("w") as fp:
